namespace KiWhisky.FrutiLogicPlatform.API.OrderManagement.Application.Internal.CommandServices;
...
using CatalogId = KiWhisky.FrutiLogicPlatform.API.Shared.Domain.Model.ValueObjects.CatalogId;
using PurchaseOrderId = KiWhisky.FrutiLogicPlatform.API.Shared.Domain.Model.ValueObjects.PurchaseOrderId;
...
public class SalesOrderCommandService : ISalesOrderCommandService
{
    ...
    public async Task<SalesOrder> Handle(GenerateSalesOrderCommand command)
    {
        ...
        try
        {
            var catalog = await catalogQueryService.Handle(
                new KiWhisky.FrutiLogicPlatform.API.ProcurementOrdering.Domain.Model.Queries.GetCatalogByIdQuery(command.catalogToBuyFrom.GetId())
            );
    ...
    public async Task<SalesOrder> GeneratePurchaseOrder(GeneratePurchaseOrderRequest request, string accountId)
    {
        ...
        if (created.SupplierId == null)
        {
            ...
            {
                var catalog = await catalogQueryService.Handle(
                    new KiWhisky.FrutiLogicPlatform.API.ProcurementOrdering.Domain.Model.Queries.GetCatalogByIdQuery(request.CatalogToBuyFrom)
                );
                var supplierOwner = catalog?.GetOwnerAccount();
                if (supplierOwner != null)
                {
